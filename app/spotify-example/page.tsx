...
import { Artist, ItemTypes, SearchResults, SpotifyApi, Page, Track } from "@spotify/web-api-ts-sdk"; // use "@spotify/web-api-ts-sdk" in your own project
...
function SpotifySearch({ sdk }: { sdk: SpotifyApi }) {
  ...
  const [userSongs, setUserSongs] = useState<Page<Track>>({} as Page<Track>);

  useEffect(() => {
    (async () => {
      const res = await sdk.currentUser.topItems("tracks");
      const results = await sdk.search("The Beatles", []);
      setUserSongs(() => res);
    })();
  }, [sdk]);
